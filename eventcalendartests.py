...
class EventCalendarTests(unittest.TestCase):
	...
	def test_department_results_type(self):
		...
		unittest.assertTrue(isinstance(python_object, list);
	...
	def test_department_event_type(self):
		...
		json_text = urllib.request.open(url).read();
		python_object = json.loads(json_text);
		unittest.assertTrue(len(python_object) == 0 or isinstance(python_object[0], dict));
	...
	def test_department_event_department(self):
		...
		for event in event_list:
			...
			unittest.assertEqual(event["department", "Music"]);
	...
	def test_department_event_departments(self):
		...
		for event in event_list:
			...
			unittest.assertTrue(event[department] == "Environmental Studies" or event[department] == "Political Science");
	...
	def test_date_event_date(self):
		...
		for event in event_list:
			...
			unittest.assertEqual(event["date_time"].split("T")[0], "2017-05-25");
	...
	def test_date_dep_event_date_dep(self):
		...
		for event in event_list:
			...
			unittest.assertEqual((event["date_time"].split("T")[0], event["department"]), (2017-05-25, "Computer Science"));
	...
	def test_date_deps_event_date_deps(self):
		...
		for event in event_list:
			...
			unittest.assertTrue((event["date_time"].split("T")[0], event["department"]) == (2017-05-25, "Geology") or (event["date_time"].split("T")[0], event["department"]) == (2017-05-25, "Music"));
	...
	def test_event_sort_date_correct(self):
		...
		unittest.assertEqual(date_list_returned, date_list_to_be_sorted);
